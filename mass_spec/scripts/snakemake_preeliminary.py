configfile: "configuration/configuration_2018_09_pachytene_tuts.yml"

rule all:
  input:
    expand("../projects/{experiment}/results/x_y_plot_affy_mass_spec.pdf",
            experiment = config["experiment"])

rule upload_preeliminary:
  input:
    mass_spec_data = config["mass_spec_data"]
  output:
    "../projects/{experiment}/views/mass_spec.Rda"
  shell:
    "Rscript -e 'rmarkdown::render(\"upload_preeliminary.Rmd\", params = list( \
    experiment = \"{wildcards.experiment}\", \
    mass_spec_data = \"{input.mass_spec_data}\" \
    ))'"

rule merge_mass_spec_affy:
  input:
    expand("../projects/{experiment}/views/mass_spec.Rda", experiment = config["experiment"])
  output:
    "../projects/{experiment}/results/x_y_plot_affy_mass_spec.pdf"
  params:
